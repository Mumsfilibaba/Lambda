...
namespace Lambda
{
	...
	EKey CWindowsEnvironment::ConvertKeyFromWindows(uint32 key)
	{
		return m_WindowsToLambdaKeyTable[key];
	}


	uint32 CWindowsEnvironment::ConvertKeyFromEngine(EKey key)
	{
		return m_LambdaToWindowsKeyTable[key];
	}


	void CWindowsEnvironment::InitializeLookUpTables()
	{
		//Set to zero
